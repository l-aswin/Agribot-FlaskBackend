...
class IoTCommand(db.Model):
    """Pending commands for the edge device to consume via /api/iot_device/command."""
    __tablename__ = 'iot_commands'
    id = db.Column(db.Integer, primary_key=True)
    device_id = db.Column(db.String(50), nullable=False, index=True)
    command = db.Column(db.String(50), nullable=False)   # start | stop | update | start_pending_upload | stop_pending_upload
    payload_json = db.Column(db.Text, default='{}')
    status = db.Column(db.String(20), default='pending', nullable=False)   # pending | consumed
    created_at = db.Column(db.DateTime(timezone=True), default=lambda: datetime.now(timezone.utc))
